#language: ru
  @Feature
  Функционал: Поиск на авито
  Сценарий: Найдем самые дорогие принтеры на авито

  Пусть открыт ресурс авито
  И в выпадающем списке категорий выбрана оргтехника
  И в поле поиска введено значение принтер
  Тогда кликнуть по выпадающемему списку региона
  Тогда в поле регион введено значение Владивосток
  И нажата кнопка показать объявления
  Тогда открылась страница результаты по запросу принтер
  И активирован чекбокс только с фотографией
  И в выпадающем списке сортировка выбрано значение Дороже
  И в консоль выведено значение названия и цены и 3 первых товаров

  @Feature1
  Структура сценария: Найдем самые дорогие принтеры на авито с таблицей
    И в выпадающем списке категорий выбрана <категория>
    И в поле поиска введено <значение>
    Тогда кликнуть по выпадающемему списку региона
    Тогда в поле регион введено значение <регион>
    И нажата кнопка показать объявления
    Тогда открылась страница результаты по запросу <значение>
    И активирован чекбокс только с фотографией
    И в выпадающем списке сортировка выбрано значение <сортировка>
    И в консоль выведено значение названия и цены и <количество> первых товаров

    Примеры:
      | категория    | значение | регион        | сортировка | количество |
      | оргтехника   | принтер  | Владивосток   | дороже     | 3          |
      | недвижимость | квартиры | Москва        | дешевле    | 5          |